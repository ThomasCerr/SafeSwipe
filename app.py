...
# Hardcoded default model (public, works on Render)
...
def verdict_label(ai_prob: Optional[float], heuristic_risk: int) -> str:
    ...
    adj = min(0.15, heuristic_risk / 100.0)
    p = base + adj
    if p >= 0.85:
...
async def analyze(request: Request,
    ...
    if c_hits:
        signals.append("Bio uses common clichés: " + ', '.join(c_hits[:4]))

    heuristic_risk = 0
    if any('near-duplicates' in s.lower() for s in signals): heuristic_risk += 12
    if c_hits: heuristic_risk += min(12, 4 * len(c_hits))
